...
exports.detectTelemetryEvents = functions.region("europe-west1").pubsub.topic("iot-topic").onPublish(
    (message, context) => {
      ...
      const documentRef = db.collection(`devices/${deviceId}/telemetry-data`).doc();
...
// Definition of the Cloud Function reacting to publication on the telemetry topic:
exports.detectTelemetryEventsTestTopic = functions.region("europe-west1").pubsub.topic("iot-test-topic").onPublish(
    (message, context) => {
      // The onPublish() trigger function requires a handler function that receives
      // 2 arguments: one related to the message published and
      // one related to the context of the message.
      ...
      // Firebase SDK for Cloud Functions has a 'json' helper property to decode
      // the message.
      const soil_humidity = message.json.soil_humidity;
      ...
      // add key for err, if it exists
      if ("err" in message.json) {
        entry.err = message.json.err;
      }
      ...
      // add key for new_soil_humidity, if it exists
      if ("new_soil_humidity" in message.json) {
        entry.new_soil_humidity = message.json.new_soil_humidity;
      }
